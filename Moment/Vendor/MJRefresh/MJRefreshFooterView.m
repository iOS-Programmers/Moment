...
- (void)adjustFrame
{
    ...
    // 设置边框
    self.frame = CGRectMake(0, y, _scrollView.frame.size.width, kRefreshViewHeight);
	...
	switch (_state = state)
    {
        ...
        case RefreshStateRefreshing:
        {
            ...
            [UIView animateWithDuration:0.2 animations:^{
                // 1.顶部多出65的滚动范围
                UIEdgeInsets inset = _scrollView.contentInset;
                inset.bottom = self.frame.origin.y - _scrollView.contentSize.height +kRefreshViewHeight;
                _scrollView.contentInset = inset;
                
                // 2.设置滚动位置
                _scrollView.contentOffset = CGPointMake(0, self.validY + kRefreshViewHeight);
            }];
			break;
        }
